...
const Product = ({ id, description, title, rate, image, price }) => {
  ...
  const { cart, isLoading } = useSelector(getCart);
      ...
      {user && (
        ...
        >
          ...
          {itemExist === -1 && (
            <button
              onClick={toAdd}
              className={`bg-orange-400 flex gap-2 py-1  text-white pl-2 pr-4 rounded-r-full ${
                isLoading ? "opacity-0" : "opacity:100"
              } text-sm max-w-fit duration-[2s]  mb-10`}
            >
          ...
          {item && (
            <div
              className={`p-2 border text-sm w-full border-r-0 border-l-0 mb-10 flex items-center gap-4 justify-center ${
                isLoading ? "opacity-0" : "opacity:100"
              } duration-[2s]  `}
            >
              <span>in cart</span>
              <button onClick={addHandle} className="p-2 font-bold text-xl">
                +
              </button>{" "}
              {item.quantity}
              <button onClick={minusHandle} className="p-2 font-bold text-xl">
                -
              </button>
            </div>
          )}
